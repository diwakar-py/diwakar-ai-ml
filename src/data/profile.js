export const profile = {
  name: "Diwakar S",
  location: "Chennai, Tamil Nadu, India",
  email: "diwakar30122002@gmail.com",
  linkedin: "https://www.linkedin.com/in/diwakar-ai-ml",
  tagline:
    "Software Developer (Data & AI Focus) | ML/DS/DL Enthusiast | Azure Data Engineer | Python, PySpark, Databricks, ADF, Synapse, Power BI | Microsoft Certified (DP-203)",
  targetRoles: [
    "Data Engineer",
    "Data Scientist",
    "Machine Learning Engineer",
    "Generative AI Engineer",
    "AI Engineer",
  ],
  heroIntro:
    "Full Stack Developer at ExcelGoodies delivering data-intensive applications, Azure data engineering workloads, and applied ML/AI prototypes that unlock business insights.",
  resumeLink: "#",
  about: [
    "Hi, I’m Diwakar — a Data & AI focused engineer with around 2+ years of hands-on experience. I started my career as a Junior Programmer, working with Python, SQL, and data workflows, and later grew into a Software Developer role where I contributed to enterprise CRM systems, data engineering pipelines, and AI/ML solutions. I work across Python, SQL, Azure Data services (ADF, Databricks, Synapse, Data Lake), Power BI, and modern development tools. I enjoy building data-driven features, experimenting with LLMs and Generative AI, and creating end-to-end solutions that bring real value to products.",
    "I’m currently exploring opportunities in Data Engineering, Data Science, ML Engineering, and AI Engineering — roles where I can blend software engineering with data, cloud, and AI tech to build impactful systems.",
  ],

  infoHighlights: [
    { label: "Location", value: "Chennai, Tamil Nadu, India" },
    { label: "Experience", value: "2+ Years" },
    { label: "Current Role", value: "Software Developer – Data & AI Focus" },
    { label: "Open To", value: "Data / AI / ML / Cloud roles" },
  ],
  experience: [
    {
      company: "ExcelGoodies Consulting, Inc.",
      role: "Software Developer (Data & AI / Data Engineering)",
      duration: "May 2025 – Present",
      location: "Chennai, Tamil Nadu, India",
      highlights: [
        "Architected and shipped enterprise CRM applications spanning React front-ends, .NET Core APIs, and SQL Server data services with embedded analytics.",
        "Built data-heavy workflow components, orchestrating advanced Python + SQL services for classification pipelines, tagging engines, and automated reporting.",
        "Owned Azure data engineering workstreams across ADF, Databricks, Synapse, and Data Lake to productionize ingestion, transformation, and monitoring.",
        "Embedded LLM-driven helpers and GenAI features into internal tools, rapidly prototyping POCs for search, summarization, and knowledge extraction.",
        "Designed Power BI data models/dashboards consumed by leadership, integrating incremental refresh, row-level security, and governance.",
        "Led CI/CD modernization, automated schema migrations, and hardened data quality guards for mission-critical CRM and analytics workloads.",
      ],
       tags: [
         "React",
        ".NET Core",
        "SQL Server",
        "Python",
        "Azure ADF",
        "Databricks",
      "Synapse",
        "Power BI",
        "Generative AI", 
         "ETL",
        "Pipelines",
        "Azure",
      ],
    },
    {
      company: "ExcelGoodies Consulting, Inc.",
      role: "Junior Programmer (Data & AI Focus)",
      duration: "May 2023 – Apr 2025",
      location: "Chennai, Tamil Nadu, India",
      highlights: [
        "Wrote Python and SQL utilities that cleaned, validated, and pre-processed raw customer datasets for downstream analytics teams.",
        "Automated lightweight ETL steps and assisted senior engineers with building and monitoring Azure-based data pipelines.",
        "Implemented simple classification/tagging scripts plus feature extraction notebooks to support experimentation across ML workflows.",
        "Built helper services that enriched datasets, generated reports, and surfaced QA signals for analysts and data scientists.",
        "Explored AI/ML concepts through lab projects, documenting learnings on model evaluation, prompt engineering, and deployment basics.",
      ],
      tags: [
        "React",
        ".NET Core",
        "SQL Server",
        "Python",
        "Azure ADF",
        "Power BI",
        "ETL",
        "Pipelines",
        "Azure",
      ],
    },
  ],
  education: [
    {
      institution: "University of Madras",
      degree: "Master of Computer Applications (MCA) – Computer Science",
      duration: "Jan 2024 – Dec 2025",
      details: [
        "Key skills: C++, Flask, Object-Oriented Programming (OOP), Problem Solving, SQL, Machine Learning, Python.",
      ],
    },
    {
      institution: "Sri Sankara Arts and Science College",
      degree: "Bachelor’s in Computer Science",
      duration: "Jun 2020 – Apr 2023",
      details: [
        "Grade: 8.89 / 10 CGPA, First Class with Distinction.",
        "Activities: Participated in multiple inter-college hackathons and technical symposiums.",
        "Achievements: Won awards in coding competitions, idea presentations, and team hackathons.",
        "Leadership: Core organizer of an internal college hackathon; active participant in tech fests, seminars, and developer communities.",
        "Skills applied: C, C++, Flask, OOP, RDBMS, Problem Solving, SQL, MongoDB, Python.",
      ],
    },
    // {
    //   institution: 'Bharathidhaasan Metric Higher Secondary School',
    //   degree: 'Higher Secondary Education',
    //   duration: '2007 – 2020',
    //   details: [
    //     'Core focus: Programming fundamentals and problem solving (C, C++, RDBMS, Python).',
    //   ],
    // },
  ],
  certifications: [
    {
      name: "Microsoft Certified: Azure Data Engineer Associate",
      issuer: "Microsoft",
      issued: "Mar 2025",
      expires: "Mar 2026",
      credentialId: "91820C6A34749823",
      highlight: true,
    },
    {
      name: "Master’s Program in Data Science",
      issuer: "Besant Technologies",
      issued: "Jul 2025",
      credentialId: "BDZ-68335",
      highlight: true,
    },
    {
      name: "Certificate of Completion – Power BI",
      issuer: "365 Data Science",
      issued: "Nov 2025",
      credentialId: "CC-18577DD14F",
      skill: "Microsoft Power BI",
    },
    {
      name: "Python 101 for Data Science",
      issuer: "Cognitive Class (IBM)",
      issued: "Aug 2025",
      credentialId: "f61370afd546415fb0873d88728d5f92",
      skill: "Python",
      highlight: true,
    },
    {
      name: "Intro to Programming",
      issuer: "Kaggle",
      issued: "Apr 2025",
    },
    {
      name: "Introduction to Programming Using Python",
      issuer: "IIITDM Jabalpur",
      issued: "Jan 2022",
      skill: "Python",
      highlight: true,
    },
    {
      name: "Advanced CPP",
      issuer: "Spoken Tutorial, EduPyramids, SINE, IIT Bombay",
      issued: "Aug 2021",
      skill: "C++",
    },
  ],
  awards: [
    { name: "Proficiency in Programming" },
    { name: "Best Outstanding Student" },
  ],
  skills: {
    filters: [
      { label: "All", value: "all" },
      { label: "Data", value: "data" },
      { label: "ML / AI", value: "ml-ai" },
      { label: "Cloud", value: "cloud" },
      { label: "Backend", value: "backend" },
      { label: "Frontend", value: "frontend" },
    ],
    groups: [
      {
        name: "Data Engineering",
        items: [
          { name: "Python", filters: ["data", "ml-ai"] },
          { name: "PySpark", filters: ["data", "ml-ai"] },
          { name: "Databricks", filters: ["data", "ml-ai", "cloud"] },
          {
            name: "CI/CD & Data Pipelines",
            filters: ["data", "cloud", "backend"],
          },
          {
            name: "SQL Server Integration Services (SSIS)",
            filters: ["data", "backend"],
          },
          { name: "Azure SQL Database", filters: ["data", "cloud"] },
        ],
      },
      {
        name: "Data Science & Analytics",
        items: [
          { name: "Data Science", filters: ["data", "ml-ai"] },
          { name: "Data Modeling", filters: ["data"] },
          { name: "Analytics", filters: ["data"] },
          { name: "Tableau", filters: ["data"] },
        ],
      },
      {
        name: "Machine Learning",
        items: [
          { name: "Machine Learning", filters: ["data", "ml-ai"] },
          { name: "Classification & Tagging", filters: ["data", "ml-ai"] },
        ],
      },
      {
        name: "Generative AI & LLMs",
        items: [
          { name: "Generative AI", filters: ["ml-ai"] },
          { name: "RAG", filters: ["ml-ai"] },
          { name: "Fine Tuning", filters: ["ml-ai"] },
          { name: "LLM Experimentation", filters: ["ml-ai"] },
        ],
      },
      {
        name: "AI Tools & LLM Ecosystem",
        items: [
          { name: "OpenAI (ChatGPT, Codex)", filters: ["ml-ai"] },
          { name: "Claude", filters: ["ml-ai"] },
          { name: "Gemini", filters: ["ml-ai"] },
          { name: "Perplexity", filters: ["ml-ai"] },
          { name: "AI API Integration", filters: ["ml-ai", "backend"] },
        ],
      },
      {
        name: "SQL & Databases",
        items: [
          { name: "SQL", filters: ["data", "backend"] },
          { name: "Microsoft SQL Server", filters: ["data", "backend"] },
          { name: "Azure SQL", filters: ["data", "cloud"] },
          { name: "RDBMS Concepts", filters: ["data"] },
          { name: "MongoDB", filters: ["data", "backend"] },
          { name: "CosmosDB", filters: ["data", "backend", "cloud"] },
        ],
      },
      {
        name: "Azure Data & Cloud Platform",
        items: [
          { name: "Azure Data Factory (ADF)", filters: ["data", "cloud"] },
          { name: "Azure Synapse Analytics", filters: ["data", "cloud"] },
          { name: "Azure Data Lake", filters: ["data", "cloud"] },
          { name: "Microsoft Azure", filters: ["cloud"] },
        ],
      },
      {
        name: "Business Intelligence & Reporting",
        items: [
          { name: "Microsoft Power BI", filters: ["data"] },
          { name: "Microsoft Excel", filters: ["data"] },
          { name: "Tableau", filters: ["data"] },
        ],
      },
      {
        name: "Cloud, DevOps & Platform Tools",
        items: [
          { name: "Azure Functions", filters: ["cloud", "backend"] },
          { name: "Containers / Docker", filters: ["cloud", "backend"] },
          { name: "Postman API", filters: ["backend"] },
        ],
      },
      {
        name: "Frontend & Core Engineering",
        items: [
          { name: "React.js", filters: ["frontend"] },
          { name: "HTML5", filters: ["frontend"] },
          { name: "CSS", filters: ["frontend"] },
          { name: "JavaScript", filters: ["frontend"] },
          { name: "C#", filters: ["backend"] },
          { name: "C", filters: ["backend"] },
          { name: "C++", filters: ["backend"] },
          { name: "Flask", filters: ["backend"] },
          { name: "Object-Oriented Programming (OOP)", filters: ["backend"] },
          { name: "Git & GitHub", filters: ["backend", "frontend"] },
          { name: "Problem Solving", filters: ["backend", "data"] },
        ],
      },
    ],
  },
  contact: {
    email: "diwakar30122002@gmail.com",
    linkedin: "https://www.linkedin.com/in/diwakar-ai-ml",
  },
};
